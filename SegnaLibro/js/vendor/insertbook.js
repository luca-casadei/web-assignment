document.addEventListener("DOMContentLoaded", () => {
    ...
    const customAuthorInput = document.querySelector(
    ...
    async function handleFormSubmit(event) {
        ...
        const genresCheckboxes = document.querySelectorAll(
        ...
        if (!isValidISBN(parts)) {
            alert(
                "ISBN non valido! Assicurati che sia nel formato EAN-CodiceRegGroup-CodiceEditoriale-CodiceTitolo-CifraControllo"
            );
            return;
        }
        ...
        const bookData = {
            ...
            Descrizione: document
                .getElementById("book_desription")
                .value.trim(),
            DataPubblicazione: document
                .getElementById("book_publish_date")
                .value.trim(),
        ...
        if (authorSelect.value === "custom") {
            [authorName, ...authorLastName] = customAuthorInput.value
                .trim()
                .split(" ");
        } else {
            [authorName, ...authorLastName] = authorSelect.options[
                authorSelect.selectedIndex
            ].text
                .trim()
                .split(" ");
        }
        ...
        const categoryData = {
            Nome: categorySelect.options[
                categorySelect.selectedIndex
            ].text.trim(),
        };

        const genresData = Array.from(genresCheckboxes).map(
        ...
        try {
            ...
            try {
                ...
                if (ex.message.toLowerCase().includes("duplicate")) {
                    alert("Un libro con queso ISBN è già stato inserito");
                }
            }
        } catch (error) {
